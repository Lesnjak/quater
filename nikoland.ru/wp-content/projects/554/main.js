 (function(){
  var domPrRoot = document.querySelector('.larus');
  ...
  // Parallax
  ;(function () {
      var domPlxNodes = document.querySelectorAll('.larus [data-plx-styles]'),
          parsedElemsData = [],
          scroll = getScroll(),
      ...
      function onScroll(e) {
        ...
        parsedElemsData.forEach(function (item) {
            ...
            if(item.styles['margin-bottom']){
              var isReverse = parseInt(item.styles['margin-bottom']) > 0,
                cssVal = item.speed * actualScroll;

              if(isReverse){
                cssVal = cssVal * -1;
              }
              item.element.style['margin-bottom'] = cssVal + 'px';
            }
            ...
            if(item.styles['x-px'] || item.styles['y-px'] || item.styles['z-px']){
                var x = y = z = 0;
                    if(item.styles['x-px']){
                      x = item.speed * actualScroll;
                      parseInt(item.styles['x-px']) > 0 && (x = x * -1);
                    }
                    if(item.styles['y-px']){
                      y = item.speed * actualScroll;
                      parseInt(item.styles['y-px']) > 0 && (y = y * -1);
                    }
                    if(item.styles['z-px']){
                      z = item.speed * actualScroll;
                      parseInt(item.styles['z-px']) > 0 && (z = z * -1);
                    }
                item.element.style['transform'] = 'translate3d(' + x + 'px, ' + y + 'px, ' + z + 'px)';
            }
        })
      }
  })();
